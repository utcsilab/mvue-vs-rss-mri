...
# Loss modules
nmse_loss = lambda x, y: np.sum(np.square(np.abs(x - y))) / np.sum(np.square(np.abs(x)))
...
for mask_type, mask_dir in itertools.product(hparams.mask_type, hparams.mask_dir):
    ...
    for ds_idx, ds in tqdm(enumerate(hparams.downsampling)):
        ...
        for sample_idx in tqdm(range(num_samples)):
            ...
            # Get recon object
            est_img_kernel = bart(1, 'pics -l1 -S -r%.5f' % l2_lam,
                     under_ksp.transpose((1, 2, 0))[:, :, None , ..., None],
                     local_mps.transpose((1, 2, 0))[:, :, None , ..., None])
            # Estimated output
            est_out = sp.resize(est_img_kernel, local_gt_rss.shape)
            ...
            # Verbose
            print('Sample %d. MVUE SSIM %.3f, RSS SSIM %.3f' % (
                sample_idx, mvue_ssim[ds_idx, sample_idx],
                rss_ssim[ds_idx, sample_idx]))
        ...
        with h5py.File(local_file, 'w') as hf:
            hf.create_dataset('saved_rec_out', data=np.asarray(saved_rec_out).astype(np.complex64))
