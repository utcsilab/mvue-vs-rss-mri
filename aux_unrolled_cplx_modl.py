...
from aux_torch_cplx_ops import TorchHybridSense, TorchHybridImage
from aux_torch_cplx_ops import TorchMoDLSense, TorchMoDLImage
from aux_utils import fft, ifft

from deepinpy.opt import ZConjGrad
...
class MoDLDoubleUnroll(torch.nn.Module):
    def __init__(self, hparams):
        ...
        self.batch_size = hparams.batch_size # Has to be fixed
        ...
        for idx in range(self.batch_size):
            if self.mode == 'DeepJSense':
                ...
                if direction == 'ConvSense':
                    ...
                    forward_op, adjoint_op, normal_op = \
                        TorchHybridImage(mps_kernel_shape,
    ...
    def forward(self, data, meta_unrolls=1):
        ...
        # Get number of coils
        num_coils      = ksp.shape[1]
        coil_mps_shape = tuple([num_coils] + self.mps_kernel_shape)
        ...
        with torch.no_grad():
            ...
            if self.map_init == 'fixed':
                ...
                # From dataloader
                est_maps_kernel = data['init_maps']
            ...
            if self.img_init == 'fixed':
                ...
                self.get_core_torch_ops(est_maps_kernel, None, 
                                        mask, 'ConvSense', coil_mps_shape)
                adjoint_batch_op = self.get_batch_op(adjoint_ops, self.batch_size)
                # Apply
                est_img_kernel = adjoint_batch_op(ksp).type(torch.cfloat)
        ...
        for meta_idx in range(meta_unrolls):
            ...
            if self.block1_max_iter > 0:
                ...
                # Normalization
                if self.map_norm:
                    est_maps_kernel = est_maps_kernel / torch.norm(
                        est_maps_kernel, p=2, dim=(0, 1), keepdim=True)
            ...
            # Apply denoising network in image space
            ...
            if self.logging:
                ...
                if meta_idx < meta_unrolls - 1:
                    _, _, scratch_ops = \
                    self.get_core_torch_ops(est_maps_kernel, None,
                                            self.ones_mask, 'ConvSense', coil_mps_shape)
        ...
        if self.logging:
            ...
            # Permute logs
            before_maps = np.stack(np.asarray(before_maps))[:, 0]
            after_maps  = np.stack(np.asarray(after_maps))[:, 0]
            # Convert to complex
            if self.use_map_net:
                before_maps = before_maps[:, :self.num_coils] + 1j * before_maps[:, self.num_coils:]
                after_maps = after_maps[:, :self.num_coils] + 1j * after_maps[:, self.num_coils:]
        
        if self.logging:
            return est_img_kernel, est_maps_kernel, est_ksp, \
                mps_logs, img_logs, mps_kernel_denoised, img_kernel_denoised, \
                ksp_logs, before_maps, after_maps
        else:
            return est_img_kernel, est_maps_kernel, est_ksp
